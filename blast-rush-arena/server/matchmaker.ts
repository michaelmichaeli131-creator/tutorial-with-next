...
const MATCH_DURATION_MS = 100_000;
...
export class Matchmaker {
    ...
    socket.addEventListener("message", (event) => {
      ...
      if (message.type === "hello") {
        if (player) {
          ...
          this.send(socket, { type: "welcome", playerId: player.id, name: player.name });
          const room = this.roomOf(player);
          if (room) this.broadcast(room, { type: "room_joined", code: room.code, players: room.players.map(publicPlayer) });
        } else {
          player = this.register(socket, message.playerId, message.name);
        }
        return;
      }
  ...
  private register(socket: WebSocket, requestedId?: string, name?: string): PlayerState {
    ...
    this.send(socket, { type: "welcome", playerId: id, name: player.name, reconnected: Boolean(previous) });
    const room = this.roomOf(player);
    if (room) {
      this.broadcast(room, { type: "player_connection", playerId: player.id, connected: true });
      this.send(socket, { type: "room_joined", code: room.code, players: room.players.map(publicPlayer), reconnected: true });
  ...
  private handle(player: PlayerState, message: ClientMessage): void {
    switch (message.type) {
      ...
      case "create_room": return this.createRoom(player);
  ...
  private quickMatch(player: PlayerState): void {
    ...
    const room = this.makeRoom([opponent, player]);
    this.startCountdown(room);
  }

  private createRoom(player: PlayerState): void {
    this.leaveRoom(player);
    const room = this.makeRoom([player]);
    this.send(player.socket, { type: "room_created", code: room.code, players: room.players.map(publicPlayer) });
  }
  ...
  private joinRoom(player: PlayerState, code: string): void {
    ...
    this.broadcast(room, { type: "room_joined", code: room.code, players: room.players.map(publicPlayer) });
    this.startCountdown(room);
  }

  private makeRoom(players: PlayerState[]): Room {
  ...
  private score(player: PlayerState, message: Extract<ClientMessage, { type: "score" }>): void {
    ...
    const pressureGain = event === "perfect" ? 16 : event === "chain" ? 20 : event === "boss" ? 25 : Math.min(8, Math.ceil(Math.max(0, delta) / 900));
